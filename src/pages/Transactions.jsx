...
export default function TransactionsPage() {
  ...
  const handleAddTransaction = async (tx) => {
    ...
    showModal({
      type: 'confirm',
      title: isIncome ? 'Confirm Income' : 'Confirm Expense',
      message: isIncome ? 'Add this income transaction?' : 'Add this expense transaction?',
      details: `Amount: ₹${tx.amount.toLocaleString('en-IN')} | ${isIncome ? 'Type: ' + tx.description : 'Category: ' + tx.category}`,
      confirmText: isIncome ? 'Add Income' : 'Add Expense',
      cancelText: 'Cancel',
      isDangerous: false,
  ...
  const handleConfirmAdd = async () => {
    ...
    try {
      ...
      showToast(
        `Transaction added: ₹${pendingTransaction.amount.toLocaleString('en-IN')} ${pendingTransaction.type === 'income' ? 'income' : 'expense'}`,
        'success'
      );
  ...
  const handleDeleteTransaction = (id, tx) => {
    ...
    showModal({
      ...
      details: `Amount: ₹${tx.amount.toLocaleString('en-IN')} | Type: ${tx.type}`,
      confirmText: 'Delete',
      cancelText: 'Cancel',
      isDangerous: true,
